...
class PropensityScoreMatching(PropensityScoringModel):
    ...
    def get_matches(self, score, potential_matches, knn, score_name, n_neighbors):
        ...
        gt = gt[gt[score_name] <= upper_score]
        try_num = 1
        while gt.empty:
            gt = potential_matches[potential_matches[score_name] >= lower_score - max_distance * try_num]
            gt = gt[gt[score_name] <= upper_score + max_distance * try_num]
            try_num += 1
        try:
            return gt.sample(n_neighbors).index.values
        except:
            return gt.sample(n_neighbors, replace = True).index.values
